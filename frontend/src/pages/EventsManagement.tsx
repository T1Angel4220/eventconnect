...
import {
  ...
  fetchEventsByOrganizer as apiFetchEventsByOrganizer,
  updateEvent as apiUpdateEvent,
  updateEventStatuses as apiUpdateEventStatuses,
...
const EventsManagement: React.FC = () => {
  ...
  const checkTableRowSpace = (
    ...
    if (availableSpace < cellHeight + 5) {
      // 5px de margen adicional
  ...
  const drawTableHeaders = (
    ...
    headers.forEach((header, index) => {
      ...
      const centerX =
        colPositions[index] + colWidths[index] / 2 - textWidth / 2;
  ...
  const truncateText = (
    ...
    while (
      doc.getTextWidth(truncatedText + "...") > maxWidth &&
  ...
  const exportToPDF = () => {
    try {
      ...
      const inProgressEvents = filteredEvents.filter(
        (e) => e.status === "En Progreso",
      ).length;
      ...
      const colWidths = [
        totalTableWidth * 0.2, // Evento (20%) - Reducido para dar espacio a Participantes
        totalTableWidth * 0.1, // Fecha (10%)
      ...
      filteredEvents.forEach((event, index) => {
        ...
        console.log(
          `Evento ${index + 1} procesado. Nueva posicion Y: ${yPosition}`,
        );
      });
      ...
      const categoryStats = filteredEvents.reduce(
        (acc, event) => {
      ...
      categories.forEach((category, index) => {
        const barHeight =
          (categoryValues[index] / maxCategoryValue) * chartHeight;
        const barX = chartStartX + index * barWidth + 5;
        const barY = chartStartY + chartHeight - barHeight;

        // Dibujar barra
        doc.setFillColor(
          barColors[index % barColors.length][0],
        ...
        doc.text(
          category,
          barX + (barWidth - 10) / 2 - doc.getTextWidth(category) / 2,
          chartStartY + chartHeight + 5,
        );

        // Valor numérico
        doc.text(
          categoryValues[index].toString(),
          barX +
            (barWidth - 10) / 2 -
            doc.getTextWidth(categoryValues[index].toString()) / 2,
          barY - 5,
        );
      });
      ...
      const statusStats = {
        ...
        "En Progreso": filteredEvents.filter((e) => e.status === "En Progreso")
          .length,
        Finalizado: filteredEvents.filter((e) => e.status === "Finalizado")
          .length,
      };
        ...
        if (statusValues[index] > 0) {
          ...
          for (let radius = pieRadius; radius > 0; radius -= 3) {
            ...
            for (let angle = startAngle; angle <= endAngle; angle += 5) {
              ...
              const x2 =
                centerX + Math.cos(((angle + 5) * Math.PI) / 180) * radius;
              const y2 =
                centerY + Math.sin(((angle + 5) * Math.PI) / 180) * radius;
          ...
          const startX =
            centerX + Math.cos((startAngle * Math.PI) / 180) * pieRadius;
          const startY =
            centerY + Math.sin((startAngle * Math.PI) / 180) * pieRadius;
          const endX =
            centerX + Math.cos((endAngle * Math.PI) / 180) * pieRadius;
          const endY =
            centerY + Math.sin((endAngle * Math.PI) / 180) * pieRadius;
          ...
          const labelY =
            centerY + Math.sin((labelAngle * Math.PI) / 180) * labelRadius;
      ...
      statusLabels.forEach((status, index) => {
        if (statusValues[index] > 0) {
          ...
          doc.setFillColor(
            statusColors[index][0],
            statusColors[index][1],
            statusColors[index][2],
          );
      ...
      doc.setFontSize(6);
      doc.text(
        "Gen: " + new Date().toLocaleDateString("es-ES"),
        legendX,
        infoY + 8,
      );
      ...
      if (capacityAnalysis.length === 0) {
        ...
        doc.setFont("helvetica", "normal");
        doc.text(
          "No hay datos de capacidad disponibles para mostrar.",
          35,
          yPosition + 10,
        );
        yPosition += 30;
        ...
        const analysisTableWidth =
          doc.internal.pageSize.width - analysisMargin * 2;
        ...
        for (let i = 1; i < analysisColWidths.length; i++) {
          analysisColPositions.push(
            analysisColPositions[i - 1] + analysisColWidths[i - 1],
          );
        }
        ...
        const drawAnalysisHeaders = (currentY: number) => {
          ...
          analysisHeaders.forEach((header, index) => {
            const textWidth = doc.getTextWidth(header);
            const centerX =
              analysisColPositions[index] +
              analysisColWidths[index] / 2 -
              textWidth / 2;
        ...
        capacityAnalysis.forEach((analysis, index) => {
          ...
          analysisData.forEach((data, colIndex) => {
            ...
            doc.text(
              truncatedText,
              analysisColPositions[colIndex] + 4,
              yPosition + 8,
            );
          });
  ...
  const convertEventResponseToEventData = (
  ...
  const updateEventInLocalState = (
    eventId: number,
    updatedEventData: EventResponse,
  ) => {
        ...
        return prevEvents.map((event) => {
          if (event.event_id === eventId) {
            ...
            return {
              ...
              organizer_name:
                convertedData.organizer_name || event.organizer_name,
              organizer_email:
                convertedData.organizer_email || event.organizer_email,
              registered_count:
                convertedData.registered_count || event.registered_count,
            };
          }
          return event;
        });
  ...
  const loadEvents = React.useCallback(async () => {
    try {
      ...
      if (Array.isArray(eventsData)) {
        ...
        const cleanedEvents = eventsData.map((event) => {
          if (event.event_image && event.event_image.startsWith("blob:")) {
            console.log(
              `🧹 Evento ${event.event_id} tiene URL blob, limpiando...`,
            );
            return {
              ...event,
      ...
      setEvents([]);
      const errorMessage =
        e instanceof Error ? e.message : "Error cargando eventos";
      showError("Error cargando eventos", errorMessage);
    }
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(
      ...
      5 * 60 * 1000,
    ); // 5 minutos para reducir llamadas
  ...
  const validateForm = () => {
    ...
    if (!newEvent.date) {
      ...
      const eventDate = new Date(
        `${newEvent.date}T${newEvent.time || "00:00"}`,
      );
      const now = new Date();
  ...
  const handleCreateOrUpdateEvent = async () => {
    ...
    try {
      ...
      if (!newEvent.date || !newEvent.time) {
        showError("Error de validación", "La fecha y hora son requeridas");
        return;
      }
      ...
      if (isNaN(testDate.getTime())) {
        showError("Error de validación", "La fecha ingresada no es válida");
        return;
      }
      ...
      formData.append(
        "event_type",
        mapUiCategoryToEventType(newEvent.category),
      );
      ...
      try {
        ...
        if (!errorMessage.includes("401") && !errorMessage.includes("403")) {
          console.warn(
            "⚠️ Error actualizando estados después de crear/editar:",
            errorMessage,
          );
        }
      }
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Error guardando evento";
      showError("Error al guardar evento", errorMessage);
    }
  };
  ...
  const handleEditEvent = (event: {
    ...
    setNewEvent({
      ...
      category: mapEventTypeToUiCategory(
        event.event_type as "academico" | "cultural" | "deportivo",
      ),
  ...
  const confirmDeleteEvent = async () => {
    ...
    try {
      ...
      try {
        ...
        if (!errorMessage.includes("401") && !errorMessage.includes("403")) {
          console.warn(
            "⚠️ Error actualizando estados después de eliminar:",
            errorMessage,
          );
        }
      }
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Error eliminando evento";
      showError("Error al eliminar evento", errorMessage);
    }
  };
  ...
  const uiEvents = React.useMemo(() => {
    ...
    return events.map((e: EventData) => {
      ...
      if (typeof e.event_date === "string") {
        ...
        console.log(
          `🔧 Fecha corregida para ${e.title}: ${e.event_date} -> ${dt.toISOString().split("T")[0]}`,
        );
      } else {
      ...
      return {
        ...
        status: statusText,
        category: mapEventTypeToUiCategory(
          e.event_type as "academico" | "cultural" | "deportivo",
        ),
  ...
  const filteredEvents = uiEvents.filter((event) => {
    // General search filter (existing functionality)
    ...
    const matchesEventSearch = eventSearchTerm === "" ||
      event.name.toLowerCase().includes(eventSearchTerm.toLowerCase());

    const matchesEventDateRange = (() => {
      if (eventDateFrom === "" && eventDateTo === "") return true;
      
      const eventDate = new Date(event.raw.event_date);
    ...
    const matchesEventType = eventTypeFilter === "" || 
      event.category === eventTypeFilter;

    return matchesGeneralSearch && matchesGeneralCategory && 
           matchesEventSearch && matchesEventDateRange && matchesEventType;
  });
  ...
  function mapEventTypeToUiCategory(
    type: "academico" | "cultural" | "deportivo",
  ): string {
    switch (type) {
      case "academico":
        return "Académico";
  ...
  function mapUiCategoryToEventType(
    category: string,
  ): "academico" | "cultural" | "deportivo" {
    switch (category) {
      case "Académico":
        return "academico";
      case "Cultural":
    ...
    <div className="min-h-screen bg-white dark:bg-black transition-colors duration-300">
      ...
      <div className="lg:ml-64">
        ...
        <main className="p-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="relative category-dropdown">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-3 h-3 ml-2 text-gray-600 dark:text-gray-400 transition-transform duration-200 ${showCategoryDropdown ? "rotate-180" : ""}`}
                    />
                  </button>
          ...
          <div className="bg-white dark:bg-black border-2 border-gray-200 dark:border-white rounded-2xl p-6 shadow-lg mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-white border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200"
              >
                Limpiar
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-800 text-black dark:text-white placeholder-gray-500 dark:placeholder-gray-400 transition-colors duration-200"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-800 text-black dark:text-white transition-colors duration-200"
                />
              </div>
            ...
            <div className="hidden lg:block">
              ...
              <div className="space-y-3">
                {filteredEvents.map((event) => (
                  <div
                    key={event.id}
                    className="grid grid-cols-9 gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-200"
                  >
                    <div className="flex items-center">
                      <div className="flex items-center space-x-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <h4 className="font-semibold text-black dark:text-white text-sm leading-tight line-clamp-2">
                            {event.name}
                          </h4>
            ...
            <div className="hidden md:block lg:hidden">
              ...
              <div className="space-y-3">
                {filteredEvents.map((event) => (
                  <div
                    key={event.id}
                    className="grid grid-cols-6 gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-200"
                  >
                    <div className="flex items-center">
                      <div className="flex items-center space-x-3">
                        {event.event_image && (
            ...
            <div className="block md:hidden">
              <div className="space-y-4">
                {filteredEvents.map((event) => (
                  <div
                    key={event.id}
                    className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-200"
                  >
                    ...
                    <div className="flex items-start space-x-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-black dark:text-white text-base mb-1 leading-tight line-clamp-2">
                          {event.name}
                        </h4>
                    ...
                    <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        ...
                        <div className="flex flex-wrap items-center gap-2 min-w-0">
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium bg-gradient-to-r ${getCategoryColor(event.category)} text-white whitespace-nowrap max-w-[120px] truncate`}
                            title={event.category}
                          >
                            {event.category}
                          </span>
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(event.status)} whitespace-nowrap`}
                          >
                            {event.status}
                          </span>
                        </div>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="bg-gradient-to-r from-blue-50/80 via-indigo-50/80 to-purple-50/80 dark:from-blue-900/20 dark:via-indigo-900/20 dark:to-purple-900/20 rounded-xl p-4 border border-blue-200/50 dark:border-blue-700/50 backdrop-blur-sm shadow-lg">
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusColor(selectedEvent.status)} shadow-lg`}
                  >
                    {selectedEvent.status}
                  </span>
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${getCategoryColor(selectedEvent.category)} text-white shadow-lg`}
                  >
              ...
              {selectedEvent.description && (
                <div className="bg-gradient-to-r from-indigo-50/80 to-purple-50/80 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-xl p-4 border border-indigo-200/50 dark:border-indigo-700/50 backdrop-blur-sm shadow-lg">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed bg-white/60 dark:bg-gray-800/60 p-3 rounded-lg backdrop-blur-sm">
                    {selectedEvent.description}
                  </p>
                </div>
              )}
      ...
      <ConfirmModal
        ...
        title={
          editingEvent?.event_id ? "Cancelar edición" : "Cancelar creación"
        }
        message={
          editingEvent?.event_id
        ...
        cancelText={
          editingEvent?.event_id ? "Continuar editando" : "Continuar creando"
        }
        type="warning"
      />
