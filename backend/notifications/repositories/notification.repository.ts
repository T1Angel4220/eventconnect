...
import { NotificationData, NotificationRow } from "../models/notification.interface";
...
class NotificationRepositoryImpl implements NotificationRepository {
  async create(notification: NotificationData): Promise<NotificationRow> {
    const query = `
      ...
      RETURNING *
    `;
    const values = [notification.user_id, notification.message, notification.status || 'pending'];
