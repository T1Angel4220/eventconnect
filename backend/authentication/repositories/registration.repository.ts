...
export interface RegistrationRepository {
  ...
  getStatsByOrganizer(organizerId: number): Promise<RegistrationStats>;
...
class RegistrationRepositoryImpl implements RegistrationRepository {
  ...
  // Obtener estadísticas de inscripciones por organizador
  async getStatsByOrganizer(organizerId: number): Promise<RegistrationStats> {
    // Estadísticas generales por organizador
    const generalStatsQuery = `
      SELECT 
        COUNT(*) as total_registrations,
        COUNT(CASE WHEN r.status = 'registered' THEN 1 END) as active_registrations,
        COUNT(CASE WHEN r.status = 'canceled' THEN 1 END) as canceled_registrations
      FROM registrations r
      JOIN events e ON r.event_id = e.event_id
      WHERE e.organizer_id = $1
    `;
    
    // Estadísticas por evento del organizador
    const eventStatsQuery = `
      SELECT 
        e.event_id,
        e.title as event_title,
    ...
    // Estadísticas por usuario (solo inscripciones en eventos del organizador)
    const userStatsQuery = `
      SELECT 
        u.user_id,
        CONCAT(u.first_name, ' ', u.last_name) as user_name,
      ...
      FROM users u
      LEFT JOIN registrations r ON u.user_id = r.user_id AND r.status = 'registered'
      LEFT JOIN events e ON r.event_id = e.event_id AND e.organizer_id = $1
      GROUP BY u.user_id, u.first_name, u.last_name
    ...
    const [generalResult, eventResult, userResult] = await Promise.all([
      pool.query(generalStatsQuery, [organizerId]),
      pool.query(eventStatsQuery, [organizerId]),
      pool.query(userStatsQuery, [organizerId])
    ]);
