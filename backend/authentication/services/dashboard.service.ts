...
export class DashboardService {
  async getDashboardStats(organizerId?: number): Promise<DashboardStats> {
    try {
      const [eventStats, registrationStats, userStats] = await Promise.all([
        organizerId
          ? eventRepository.getStatsByOrganizer(organizerId)
          : eventRepository.getStats(),
        organizerId
          ? registrationRepository.getStatsByOrganizer(organizerId)
      ...
      return {
        ...
        recent_registrations: parseInt(
          registrationStats.active_registrations.toString(),
        ),
      };
    } catch (error) {
  ...
  async getDashboardStatsWithGrowth(
    organizerId?: number,
  ): Promise<DashboardStats & { growth: any }> {
    try {
  ...
  async getRecentEventsByOrganizer(
    organizerId: number,
    limit: number = 10,
  ...
  > {
    try {
      ...
      events.forEach((event) => {
        categoryCount[event.event_type] =
          (categoryCount[event.event_type] || 0) + 1;
      });
  ...
  async getParticipantsByMonth(
    months: number = 6,
  ): Promise<
    Array<{ month: string; year: number; participants: number; events: number }>
  > {
    try {
